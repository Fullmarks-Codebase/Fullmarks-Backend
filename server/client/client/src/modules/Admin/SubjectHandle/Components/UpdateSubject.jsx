...
export default function UpdateSubject(props) {
  ...
  React.useEffect(() => {
    axiosForAdmin
      .post("/subjects/onlySubjects", { subjectId: id, classId })
      .then((response) => {
  ...
  const updateSubject = (e) => {
    ...
    axiosForAdmin
      .put("/subjects/updateSubject", formData, config)
      .then((response) => {
  ...
  const deleteOldImage = (event) => {
    ...
    axiosForAdmin
      .post("/subjects/deleteImage", { id, image_field: name })
      .then((res) => {
        forceUpdate((n) => !n);
      })
      .catch((err) => {
                            ...
                            {selected.image ? (
                              <>
                                <div className='col-md-8 col-12'>
                                  ...
                                  <img
                                    src={`${process.env.REACT_APP_NODE_URL_SUBJECTS}/${selected.image}`}
                                    alt={selected.image}
                                    width='100%'
                                  />
                                </div>
